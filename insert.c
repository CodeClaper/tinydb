...
static uint32_t get_column_size(InsertNode *insert_node) {
    return insert_node->columns_set_node->ident_set_node->num;
}

static char *get_column_name(InsertNode *insert_node, uint32_t index) {
...
Row *generate_insert_row(InsertNode *insert_node) {
    ...
    row->data_len = get_column_size(insert_node);
    for(uint32_t i = 0; i < row->data_len; i++) {
        KeyValue *key_value = malloc(sizeof(KeyValue));
        key_value->key = get_column_name(insert_node, i);
