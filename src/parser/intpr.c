...
void add_assignment_to_set(AssignmentSetNode *assignment_set_node,
                           AssignmentNode *assignment_node) {
  assignment_set_node->assignment_node =
      db_realloc(assignment_set_node->assignment_node,
                 sizeof(AssignmentNode *) * (assignment_set_node->num + 1));
  *(assignment_set_node->assignment_node + assignment_set_node->num) =
      assignment_node;
...
void add_table_ref_to_set(TableRefSetNode *table_ref_set,
                          TableRefNode *table_ref) {
  table_ref_set->set = db_realloc(
      table_ref_set->set, sizeof(TableRefNode *) * (table_ref_set->size + 1));
