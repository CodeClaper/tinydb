...
uint32_t get_internal_node_cell_child_page_num(void *node, void *key, uint32_t keys_num, uint32_t key_len, uint32_t default_value_len, DataType key_data_type) {
    /* Binary search */
    uint32_t min_index = 0;
    uint32_t max_index = keys_num;
    while(min_index != max_index) {
...
void initial_leaf_node(void *leaf_node, uint32_t default_value_len, bool is_root) {
    ...
    set_leaf_node_next_leaf(leaf_node, default_value_len, 0); // 'next leaf = 0' means no subling leaf node
}
...
static bool overflow_internal_node(void *internal_node, uint32_t keys_num, uint32_t key_len, uint32_t value_len) {
    uint32_t cell_len = key_len + INTERNAL_NODE_CELL_CHILD_SIZE;
...
static void copy_root_to_internal_node(void *root, void *internal_node, uint32_t key_len, uint32_t value_len) {
...
static void insert_and_split_internal_node(Table *table, uint32_t old_internal_page_num, uint32_t new_child_page_num) {
    ...
    uint32_t new_child_max_key_index = get_internal_node_key_index(old_internal_node, new_child_max_key, keys_num, key_len, value_len, primary_key_meta_column->column_type);
    ...
    set_internal_node_right_child(new_internal_node, value_len, get_internal_node_right_child(old_internal_node, value_len));
    ...
    set_internal_node_right_child(old_internal_node, value_len, get_internal_node_child(old_internal_node, LEFT_SPLIT_COUNT - 1, key_len, value_len));
...
static void *get_replaced_child_node(Table *table, void *node, uint32_t default_value_len) {
    ...
    if (get_node_type(node) == INTERNAL_NODE) {
        ...
        if (keys_num == 1 && right_child_page_num == 0) {
            uint32_t key_len = calc_primary_key_length(table);
            uint32_t child_page_num = get_internal_node_child(node, 0, key_len, default_value_len);
            child_node = get_page(table->meta_table->table_name, table->pager, child_page_num);
            return get_replaced_child_node(table, child_node, default_value_len);
...
void delete_internal_node_cell(Table *table, uint32_t page_num, void *key, DataType key_data_type) {
    ...
    void *max_key = key_num == 0 ? NULL : get_internal_node_key(internal_node, key_num - 1, key_len, value_len);
    ...
    if (!is_root_node(internal_node) && key_num == 0 && get_internal_node_right_child(internal_node, value_len) == 0)
...
void root_fall_back_root_node(Table *table) {
    ...
    if (get_node_type(root) == INTERNAL_NODE) {
        void *replace_node = get_replaced_child_node(table, root, value_len);
        ...
        switch (node_type) {
            case LEAF_NODE: {
                set_node_type(root, LEAF_NODE);
                uint32_t cells_num = get_leaf_node_cell_num(replace_node, value_len); 
            ...
            case INTERNAL_NODE: {
                set_node_type(root, INTERNAL_NODE);
                uint32_t keys_num = get_internal_node_keys_num(replace_node, value_len);
                /* May be overflow. wait for resolution. */
                ...
                set_internal_node_keys_num(root, value_len, keys_num);
                uint32_t right_child_page_num = get_internal_node_right_child(replace_node, value_len);
                set_internal_node_right_child(root, value_len, right_child_page_num);
                void *right_child = get_page(table->meta_table->table_name, table->pager, right_child_page_num);
                set_parent_pointer(right_child, table->root_page_num);
                break;
...
MetaColumn *deserialize_meta_column(void *destination) {
    ...
    switch (meta_column->default_value_type) {
        case DEFAULT_VALUE:
           meta_column->default_value = copy_value(destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE + ROOT_NODE_DEFAULT_VALUE_TYPE_SIZE, 
    ...
    meta_column->has_comment = (bool)*(uint8_t *)(destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE + ROOT_NODE_DEFAULT_VALUE_TYPE_SIZE + ROOT_NODE_DEFAULT_VALUE_SIZE);
    strcpy(meta_column->comment, destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE + ROOT_NODE_DEFAULT_VALUE_TYPE_SIZE + ROOT_NODE_DEFAULT_VALUE_SIZE + ROOT_NODE_HAS_COMMENT_SIZE);
    return meta_column;
}
...
void *serialize_meta_column(MetaColumn *meta_column) {
    ...
    *(uint8_t *)(destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE) = meta_column->default_value_type;
    switch (meta_column->default_value_type) {
        case DEFAULT_VALUE:
            memcpy(destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE + ROOT_NODE_DEFAULT_VALUE_TYPE_SIZE, 
                   meta_column->default_value, 
                   ROOT_NODE_DEFAULT_VALUE_SIZE);
            break;
        default:
            break;
    }

    *(uint8_t *)(destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE + ROOT_NODE_DEFAULT_VALUE_TYPE_SIZE + ROOT_NODE_DEFAULT_VALUE_SIZE) = meta_column->has_comment;  
    memcpy(destination + ROOT_NODE_META_COLUMN_NAME_SIZE + ROOT_NODE_META_COLUMN_TYPE_SIZE + ROOT_NODE_META_COLUMN_LENGTH_SIZE + ROOT_NODE_IS_PRIMARY_SIZE + ROOT_NODE_META_COLUMN_TABLE_NAME_SIZE + ROOT_NODE_SYS_RESERVED_SIZE + ROOT_NODE_IS_UNIQUE_SIZE + ROOT_NODE_NOT_NULL_SIZE + ROOT_NODE_ARRAY_DIM_SIZE + ROOT_NODE_ARRAY_CAP_SIZE + ROOT_NODE_DEFAULT_VALUE_TYPE_SIZE + ROOT_NODE_DEFAULT_VALUE_SIZE + ROOT_NODE_HAS_COMMENT_SIZE, 
           meta_column->comment, 
